...
def verifyToken(token, now):
    ...
    '''
    try:
        rows = cur.execute("SELECT UserID FROM Users where UserID IN(SELECT UserID FROM Tokens where TokenID = ?)", (token, ))
        rows = rows.fetchall()
        if len(rows) == 1:
            return True
        if len(rows) > 1:
...
def storeToken(token, expirydate, uid):
    ...
    '''
    cur.execute("INSERT INTO Tokens (TokenID, UserID, EXPIRYDATE) VALUES(:tid, :uid, :expirydate)", {"tid": token, "uid": uid, "expirydate": expirydate})
    cur.commit()
    return token
...
def isvalidtoken(uid, token):
    ...
    try:
        ...
        rows = rows.fetchall()
        for row in rows[0]:
            if row < timenow: # comparion between string returns True if the Token is not expired else False.
                return False
        return True
    except Error as e:
        render_template('error.html', message=f'{e}')
...
def updateToken(token, uid):
    ...
    '''
    try:
        rows = cur.execute("UPDATE Tokens SET EXPIRYDATE = ? WHERE TokenID = ? AND UserID = ?)", (generateToken(datetime.now()), token, uid))
        cur.commit()
        return True
    except Error as e:
        render_template('error.html', message=f'{e}')
...
def getUserDate():
    '''
    It Supports both Post and Get request.
    While request is Get, We just redirect to the homepage which asks the user to enter the Username, Userid and token.
    On the otherhand, We take the inputs and check if the userid is already present in our database. If so, we Intimate that his token is not expired yet.
    ...
    if request.method == 'POST':
        ...
        presentTime = datetime.now()        # Stroing the time at present.
        if verifyToken(Token, presentTime):     # If User id is found in our DB.
            if isvalidtoken(User_ID, Token):
                user = getUsername(User_ID).fetchone()      # Fetch the details(userid and username) of the user form users table.
                return render_template('error.html', message=f'Hello, {user[0]}!! Your Token is Not Expired Yet!')  # if token is not expired yet.
            else:
            ...
            try:
                ...
                cur.commit()
                Expiry = generateToken(presentTime).strftime("%m/%d/%Y, %H:%M:%S").replace(',', '')  # converting into string from datetime object for incompatability for storing the data in db.
                storeToken(Token, Expiry, User_ID)      # Insert the Expiry date and tokenid, userid in tokens table.
                return render_template('error.html', message=f'Hello, {name}!! Your Token is {Token}.')     # At last, return the token as the user is new user.
            except Error as e:      # If the enterd UserID is already exists.
                return render_template('error.html', message=f'UserID is already exists. Try changing with other UserID.')
    else:
        return render_template('homepage.html') # This fires when the request is getself.